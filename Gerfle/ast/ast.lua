...
function Ast.FunctionLiteralExpression(args,body)
    return {kind=AstKind.FunctionLiteralExpression,args=args,body=body,isConstant=false}
end
function Ast.FunctionCallExpression(base,args)
    return {kind=AstKind.FunctionCallExpression,base=base,args=args,isConstant=false}
end
function Ast.PassSelfFunctionCallExpression(base,name,args)
    return {kind=AstKind.PassSelfFunctionCallExpression,base=base,passSelfFunctionName=name,args=args,isConstant=false}
end
function Ast.VariableExpression(scope,id)
    scope:addReference(id)
    return {kind=AstKind.VariableExpression,scope=scope,id=id,isConstant=false,
        getName=function(self) return self.scope:getVariableName(self.id) end}
end
