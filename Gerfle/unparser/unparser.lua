...
local function escStr(s) return escape(s) end
...
function Unparser:unparseStatement(stmt,tabbing)
    ...
    if k==AstKind.ReturnStatement then
        push("return")
        for i,e in ipairs(stmt.args) do
            if i==1 then push(self:whitespaceIfNeeded(self:unparseExpression(e,tabbing))) end
            push(self:unparseExpression(e,tabbing))
            if i<#stmt.args then push(",",self:optionalWhitespace()) end
        end
    elseif k==AstKind.BreakStatement then
        push("break")
    elseif k==AstKind.ContinueStatement then
        push("continue")
    elseif k==AstKind.NopStatement then
        -- nothing
    elseif k==AstKind.DoStatement then
        push("do",self:newline(true),self:unparseBlock(stmt.body,tabbing+1),self:newline(true),self:tabs(tabbing,true),"end")
    elseif k==AstKind.WhileStatement then
        push("while",self:whitespaceIfNeeded(self:unparseExpression(stmt.condition,tabbing)),
            self:unparseExpression(stmt.condition,tabbing),self:whitespaceIfNeeded2(self:unparseExpression(stmt.condition,tabbing)),
            "do",self:newline(true),self:unparseBlock(stmt.body,tabbing+1),self:newline(true),self:tabs(tabbing,true),"end")
    elseif k==AstKind.RepeatStatement then
        push("repeat",self:newline(true),self:unparseBlock(stmt.body,tabbing+1),
            self:newline(true),self:tabs(tabbing,true),"until",
            self:whitespaceIfNeeded(self:unparseExpression(stmt.condition,tabbing)),
            self:unparseExpression(stmt.condition,tabbing))
    elseif k==AstKind.ForStatement then
        local init=self:unparseExpression(stmt.initialValue,tabbing)
        local fin=self:unparseExpression(stmt.finalValue,tabbing)
        local inc=self:unparseExpression(stmt.incrementBy,tabbing)
        local vname=stmt.scope:getVariableName(stmt.id)
        push("for",self:whitespace(),vname,self:optionalWhitespace(),"=",self:optionalWhitespace(),
            init,",",self:optionalWhitespace(),fin)
        if not (stmt.incrementBy.isConstant and stmt.incrementBy.value==1) then
            push(",",self:optionalWhitespace(),inc)
        end
        push(self:whitespaceIfNeeded2(inc),"do",self:newline(true),
            self:unparseBlock(stmt.body,tabbing+1),self:newline(true),self:tabs(tabbing,true),"end")
    elseif k==AstKind.ForInStatement then
        ...
        push(self:whitespace(),"in",self:whitespace())
        for i,e in ipairs(stmt.expressions) do
            if i>1 then push(",",self:optionalWhitespace()) end
            push(self:unparseExpression(e,tabbing))
        end
        push(self:whitespace(),"do",self:newline(true),
            self:unparseBlock(stmt.body,tabbing+1),self:newline(true),self:tabs(tabbing,true),"end")
    elseif k==AstKind.IfStatement then
        local cond=self:unparseExpression(stmt.condition,tabbing)
        push("if",self:whitespaceIfNeeded(cond),cond,self:whitespaceIfNeeded2(cond),"then",
            self:newline(true),self:unparseBlock(stmt.body,tabbing+1))
        for _,eif in ipairs(stmt.elseifs) do
            local ec=self:unparseExpression(eif.condition,tabbing)
            push(self:newline(true),self:tabs(tabbing,true),"elseif",
                self:whitespaceIfNeeded(ec),ec,self:whitespaceIfNeeded2(ec),"then",
                self:newline(true),self:unparseBlock(eif.body,tabbing+1))
        end
        if stmt.elsebody then
            push(self:newline(true),self:tabs(tabbing,true),"else",
                self:newline(true),self:unparseBlock(stmt.elsebody,tabbing+1))
        end
        push(self:newline(true),self:tabs(tabbing,true),"end")
    elseif k==AstKind.FunctionDeclaration then
        ...
        push("function",self:whitespace(),name,"(")
        for i,arg in ipairs(stmt.args) do
            if i>1 then push(",",self:optionalWhitespace()) end
            push(arg.kind==AstKind.VarargExpression and "..." or arg.scope:getVariableName(arg.id))
        end
        push(")",self:newline(true),self:unparseBlock(stmt.body,tabbing+1),
            self:newline(true),self:tabs(tabbing,true),"end")
    elseif k==AstKind.LocalFunctionDeclaration then
        push("local",self:whitespace(),"function",self:whitespace(),stmt.scope:getVariableName(stmt.id),"(")
        for i,arg in ipairs(stmt.args) do
            if i>1 then push(",",self:optionalWhitespace()) end
            push(arg.kind==AstKind.VarargExpression and "..." or arg.scope:getVariableName(arg.id))
        end
    ...
    elseif k==AstKind.FunctionCallStatement then
        local base=self:unparseExpression(stmt.base,tabbing)
        push(base,"(")
        for i,arg in ipairs(stmt.args) do
            if i>1 then push(",",self:optionalWhitespace()) end
    ...
    elseif k==AstKind.PassSelfFunctionCallStatement then
        local base=self:unparseExpression(stmt.base,tabbing)
        push(base,":",stmt.passSelfFunctionName,"(")
        for i,arg in ipairs(stmt.args) do
            if i>1 then push(",",self:optionalWhitespace()) end
            push(self:unparseExpression(arg,tabbing))
        end
