...
local Tokenizer = require("lexer.tokenizer")
...
local function pExpect(self,kind,src)
    ...
    if src then
        logger:error(pGenErr(self,string.format("unexpected <%s> \"%s\", expected <%s> \"%s\"",tk.kind,tk.source,kind,src)))
    else
        logger:error(pGenErr(self,string.format("unexpected <%s> \"%s\", expected <%s>",tk.kind,tk.source,kind)))
    end
end
...
function Parser:statement(scope,currentLoop)
    while pConsume(self,TokenKind.Symbol,";") do end
    if pConsume(self,TokenKind.Keyword,"break") then
        if not currentLoop then
            if self.disableLog then error() end
            logger:error(pGenErr(self,"break only valid inside loops"))
        end
        return Ast.BreakStatement(currentLoop,scope),true
    end
    if self.luaVersion=="LuaU" and pConsume(self,TokenKind.Keyword,"continue") then
        if not currentLoop then
            if self.disableLog then error() end
            logger:error(pGenErr(self,"continue only valid inside loops"))
        end
        return Ast.ContinueStatement(currentLoop,scope),true
    end
    if pConsume(self,TokenKind.Keyword,"do") then
        local body=self:block(scope,currentLoop)
        pExpect(self,TokenKind.Keyword,"end")
        return Ast.DoStatement(body)
    end
    if pConsume(self,TokenKind.Keyword,"while") then
    ...
    if pConsume(self,TokenKind.Keyword,"repeat") then
    ...
    if pConsume(self,TokenKind.Keyword,"return") then
        local args={}
        if not pIs(self,TokenKind.Keyword,"end") and not pIs(self,TokenKind.Keyword,"elseif")
            and not pIs(self,TokenKind.Keyword,"else") and not pIs(self,TokenKind.Symbol,";")
            and not pIs(self,TokenKind.Eof) then
    ...
    if pConsume(self,TokenKind.Keyword,"local") then
        if pConsume(self,TokenKind.Keyword,"function") then
        ...
        local exprs={}
        if pConsume(self,TokenKind.Symbol,"=") then exprs=self:exprList(scope) end
        self:enableNameList(scope,ids)
        return Ast.LocalVariableDeclaration(scope,ids,exprs)
    end
    if pConsume(self,TokenKind.Keyword,"for") then
        if pIs(self,TokenKind.Symbol,"=",1) then
...
function Parser:functionArgList(scope)
    local args={}
    while not pIs(self,TokenKind.Symbol,")") do
        if pConsume(self,TokenKind.Symbol,"...") then
        ...
        if not pConsume(self,TokenKind.Symbol,",") then break end
    end
    return args
end
...
function Parser:expressionAddSub(scope)
    ...
    repeat
        local found=false
        if pConsume(self,TokenKind.Symbol,"+") then
            curr=Ast.AddExpression(curr,self:expressionMulDiv(scope),true); found=true
        elseif pConsume(self,TokenKind.Symbol,"-") then
            curr=Ast.SubExpression(curr,self:expressionMulDiv(scope),true); found=true
        end
    until not found
    return curr
end
...
function Parser:expressionUnary(scope)
    if pConsume(self,TokenKind.Keyword,"not") then
...
function Parser:expressionPow(scope)
    local lhs=self:expressionFunctionCall(scope)
    if pConsume(self,TokenKind.Symbol,"^") then
